...
func handle_step_climbing():
	step_status = "No verical collision detected"
	...
	for i in player.get_slide_collision_count():
		...
		if _is_vertical_surface(collision):
			...
			if measured_height > MIN_STEP_HEIGHT and measured_height <= step_height and _is_valid_step_direction(collision):
				...
				player.velocity = player.previous_velocity # prevent stopping when hitting a climbable step
				player.camera.smooth_step(measured_height) # smooth camera while ascending step
				step_status = "Step Found - Height: " + str(measured_height)
			else:
				step_status = "Step too high- Height: " + str(measured_height)
			break
